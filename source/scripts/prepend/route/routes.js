...
Router.loadPageElement = function(path,element) {
	Router.contentRequest(path,0,"GET",function(scripts) {
		element.insertAdjacentHTML('beforeend', this);
		...
		if(scripts) {
			for(var s=0; s<scripts.length;s++){
				...
				if(scripts[s].src) {
					...
					//	document.getElementsByTagName("body")[0].insertAdjacentHTML('beforeend', script);
					document.getElementsByTagName("body")[0].appendChild( script ).parentNode.removeChild( script );
				} else{
					var script = document.createElement( "script" );
